        ...
        switch (customId) {
            ...
            case 'admin_generate_gift_card':
                if (!hasAdminRole(interaction)) {
                    const embed = new EmbedBuilder()
                        .setTitle('❌ Access Denied')
                        .setDescription('You need admin privileges to use this feature.')
                        .setColor(0xFF0000);
                    return await interaction.reply({ embeds: [embed], ephemeral: true });
                }
...
async function handleAdminGiftCardGeneration(interaction) {
    const diamondAmount = parseInt(interaction.fields.getTextInputValue('admin_diamond_amount'));
    ...
    setTimeout(async () => {
        try {
            ...
            console.log('Could not delete admin gift card generation result:', error.message);
        }
    }, 10 * 60 * 1000);
...
async function cleanupOldPanels() {
    ...
    const channels = [CHANNELS.daily_claims, CHANNELS.gambling, CHANNELS.gift_cards, CHANNELS.leaderboard, CHANNELS.information, CHANNELS.gift_card_verification];
